...
const DoctorList = () => {
  ...
  // 🔹 Filters state (ONLY filter values)
  ...
  // 🔹 UI-only state (important separation)
  const [showFilters, setShowFilters] = useState(false);

  // 🔹 RTK Query (auto fetch on filters change)
  const { data, isLoading, error } = useListDoctorsQuery(filters);
  ...
  const activeFilterCount = Object.entries(filters).filter(([key, value]) => {
    ...
    if (key === "sortBy" && value === "experience") return false;
    if (key === "sortOrder" && value === "desc") return false;
    return true;
  }).length;

  if (isLoading) return <Loader2 className="animate-spin mx-auto mt-10" />;
  if (error) return <p className="text-center mt-10">Failed to load doctors</p>;
    ...
    <div className="min-h-screen bg-gray-50 pt-16">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="mb-4 text-sm text-gray-600">
          {isLoading ? "Seaching..." : `${data.items.length} doctor found`}
        </div>
